  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden h-full transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl">
      <div className="relative">
        ...
        <div className="bg-gradient-to-br from-sinet-light to-sinet h-48 flex items-center justify-center">
          {imagePath ? (
            <img
              src={imagePath}
              alt={title}
            ...
            <div className="text-white text-xl font-bold p-6 text-center">{title}</div>
          )}
        </div>
      ...
      <div className="p-6">
        ...
        {startingPrice && (
          <div className="mb-3">
            ...
            <p className="text-2xl font-bold text-sinet-dark">${startingPrice}</p>
          </div>
        )}
        ...
        {bulletPoints && bulletPoints.length > 0 && (
          <ul className="text-gray-600 mb-4 pl-5">
            {bulletPoints.map((point, index) => (
              <li key={index} className="list-disc mb-1">{point}</li>
            ))}
          </ul>
        )}
